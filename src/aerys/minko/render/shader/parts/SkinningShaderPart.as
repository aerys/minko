	...
	public class SkinningShaderPart extends ShaderPart
	{
		...
		private function dualQuaternionSkinPosition(inPosition : SFloat) : SFloat
		{
			...
			else
			{
				...
				var tmp1	: SFloat = cross(dQnYZW, inPosition);			//	tmp1 = cross(blendDQ[0].yzw, IN.position.xyz)
				var tmp2	: SFloat = add(tmp1, tmp0);						//	tmp2 = tmp1 + tmp0
				var tmp3	: SFloat = cross(dQnYZW, tmp2);					//	tmp3 = cross(blendDQ[0].yzw, tmp2)
				...
				var tmp8	: SFloat = cross(dQnYZW, dQdYZW);				//	tmp8 = cross(blendDQ[0].yzw, blendDQ[1].yzw)
		...
		private function dualQuaternionSkinNormal(inNormal : SFloat) : SFloat
		{
			...
			else
			{
				...
				var tmp1	: SFloat = cross(dQnYZW, inNormal);		//	tmp1 = cross(blendDQ[0].yzw, IN.position.xyz)
				var tmp2	: SFloat = add(tmp1, tmp0);				//	tmp2 = tmp1 + tmp0
				var tmp3	: SFloat = cross(dQnYZW, tmp2);			//	tmp3 = cross(blendDQ[0].yzw, tmp2)
				var tmp4	: SFloat = multiply(2, tmp3);			//	tmp4 = 2.0*tmp3
