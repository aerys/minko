package aerys.minko.type.interpolation
{
	...
	{	
		...
		public function clone() : IInterpolation
		{
			...
			clone._segments = new Vector.<IInterpolation>;
			for (var segmentId : uint = 0; segmentId < numSegments; ++segmentId)
				_segments.push(_segments[segmentId].clone());
			_length = NaN;
		...
		public function updateMatrix(t : Number, matrix : Matrix4x4 = null) : Matrix4x4
		{
			...
			return segment.updateMatrix(localT, matrix);
		}
		...
		public function addSmoothLinearSegment(length : Number) : Path
		{
			...
			var end : Vector4 = 
				getControlSymmetric(length, lastSegment.lastControl, lastSegment.end);
												   ...
												   controlDistance	: Number = 0) : Path
		{
			...
			var control : Vector4 = 
				getControlSymmetric(controlDistance, lastSegment.lastControl, lastSegment.end);
													...
													control1Distance	: Number = 0) : Path
		{
			...
			var control1 : Vector4 = 
				getControlSymmetric(control1Distance, lastSegment.lastControl, lastSegment.end);
												   ...
												   control2Distance : Number = 0) : Path
		{
			...
			var control2 : Vector4 = 
				getControlSymmetric(control2Distance, firstSegment.lastControl, firstSegment.start);
		...
		private function getLocalT(t : Number) : Number
		{
			...
			return ti == numSegments ? 1.0 : t - ti;
...
//			 return (t * _segments.length) % 1;
		...
		private function getSegment(t : Number) : IInterpolation
		{
			...
			var numSegments	: int = _segments.length;
			var segmentId	: int = int(t * numSegments);
			segmentId	= segmentId < numSegments ? segmentId : numSegments - 1;
...
//			 return _segments[int(t * _segments.length)];
		...
		private function getControlSymmetric(distance	: Number, 
											control	: Vector4, 
