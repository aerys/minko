...
APP_NAME=$(sed -r 's/lib(.*).so/minko.\1/;s/([A-Za-z])([A-Za-z]+)/\U\1\L\2/g;s/([0-9]+)/_\1/g;s/[^[:alpha:]]//g' <<< "${TARGET_NAME}")
PACKAGE=$(sed -r 's/lib(.*).so/minko.\1/;s/-/\./g;s/([0-9]+)/_\1/g;s/(.*)/\L\1/' <<< "${TARGET_NAME}")
