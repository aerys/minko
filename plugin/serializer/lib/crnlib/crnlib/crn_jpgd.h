...
namespace jpgd
{
    ...
    unsigned char*
    decompress_jpeg_image_from_memory(const unsigned char* pSrc_data, int src_data_size, int* width, int* height, int* actual_comps, int req_comps, uint32_t flags = 0);
    unsigned char*
    decompress_jpeg_image_from_file(const char* pSrc_filename, int* width, int* height, int* actual_comps, int req_comps, uint32_t flags = 0);
    ...
    public:
        ...
        virtual int
        read(uint8* pBuf, int max_bytes_to_read, bool* pEOF_flag) = 0;
    };
    ...
    class jpeg_decoder_file_stream : public jpeg_decoder_stream
    {
        ...
        operator=(const jpeg_decoder_file_stream&);
    ...
    public:
        ...
        bool
        open(const char* Pfilename);
        void
        close();

        virtual int
        read(uint8* pBuf, int max_bytes_to_read, bool* pEOF_flag);
    };
    ...
    public:
        ...
        bool
        open(const uint8* pSrc_data, uint size);
        void
        close()
        {
            m_pSrc_data = NULL;
    ...
    unsigned char*
    decompress_jpeg_image_from_stream(jpeg_decoder_stream* pStream, int* width, int* height, int* actual_comps, int req_comps, uint32_t flags = 0);
    ...
    public:
        ...
        int
        decode(const void** pScan_line, uint* pScan_line_len);
        ...
        inline int
        get_bytes_per_pixel() const
        {
            return m_dest_bytes_per_pixel;
        }
        inline int
        get_bytes_per_scan_line() const
        {
    ...
    private:
        ...
        inline int
        check_sample_buf_ofs(int ofs) const
        {
            assert(ofs >= 0);
        ...
        free_all_blocks();
        JPGD_NORETURN void
        stop_decoding(jpgd_status status);
        void*
        alloc(size_t n, bool zero = false);
        void*
        alloc_aligned(size_t nSize, uint32_t align = 16, bool zero = false);
        void
        word_clear(void* p, uint16 c, uint n);
        void
        prep_in_buffer();
        void
        read_dht_marker();
        ...
        locate_sos_marker();
        void
        init(jpeg_decoder_stream* pStream, uint32_t flags);
        void
        create_look_ups();
        void
        fix_in_buffer();
        void
        transform_mcu(int mcu_row);
        coeff_buf*
        coeff_buf_open(int block_num_x, int block_num_y, int block_len_x, int block_len_y);
        inline jpgd_block_coeff_t*
        coeff_buf_getp(coeff_buf* cb, int block_x, int block_y);
        void
        load_next_row();
        void
        decode_next_row();
        void
        make_huff_table(int index, huff_tables* pH);
        void
        check_quant_tables();
        void
        check_huff_tables();
        ...
        process_restart();
        void
        decode_scan(pDecode_block_func decode_block_func);
        void
        init_progressive();
        void
        init_sequential();
        ...
        decode_start();
        void
        decode_init(jpeg_decoder_stream* pStream, uint32_t flags);
        void
        H2V2Convert();
        uint32_t
        H2V2ConvertFiltered();
        ...
        inline uint
        get_bits_no_markers(int numbits);
        inline int
        huff_decode(huff_tables* pH);
        inline int
        huff_decode(huff_tables* pH, int& extrabits);
