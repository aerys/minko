...
main(void)
{
	float r = texture2D(uTexture, vec2(vTexCoord0.x, vTexCoord0.y)).r;
    float g = texture2D(uTexture, vec2(vTexCoord1.x, vTexCoord1.y)).g;
    float b = texture2D(uTexture, vec2(vTexCoord2.x, vTexCoord2.y)).b;
